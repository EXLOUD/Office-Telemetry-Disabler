<#
Office Privacy and Telemetry (adapted for PS7)
...
Adapted for PowerShell 7, compatible with Windows 7 and newer
by EXLOUD
...
if (-not $isAdmin) {
    Write-Host "  [ERROR] Administrator privileges required." -ForegroundColor $Colors.Error
...
# Function to disable scheduled task with logging (using schtasks.exe for PS7 compatibility)
function Disable-ScheduledTaskWithLogging {
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            $taskInfo = $queryResult | Where-Object { $_ -match "Status\s+:\s+Disabled" }
            if ($taskInfo) {
...
if ($applyHostsBlock -eq 'y' -or $applyHostsBlock -eq 'Y') {
    try {
        # Skip Windows Defender exclusion on Windows 7 as Add-MpPreference is not available
        if ($windowsVersion -ne "Windows 7") {
            try {
        ...
        if ($wasReadOnly) {
            ...
            Set-ItemProperty -Path $hostsFile -Name IsReadOnly -Value $false -ErrorAction Stop
        }
        ...
        Write-Host "  [ERROR] Failed to update hosts file: $($_.Exception.Message)" -ForegroundColor $Colors.Error
